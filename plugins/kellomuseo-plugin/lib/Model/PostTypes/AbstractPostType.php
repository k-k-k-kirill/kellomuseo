...
abstract class AbstractPostType {
	...
	public function set_labels() {
		...
		$labels = array(
			'name'               => sprintf(
				_x( '%s', 'post type general name', 'kellomuseo-plugin' ),
				$this->post_label_plural
			),
			'singular_name'      => sprintf( _x( '%s', 'post type singular name', 'kellomuseo-plugin' ), $this->post_label_singular ),
			'menu_name'          => sprintf( _x( '%s', 'admin menu', 'kellomuseo-plugin' ), $this->post_label_plural ),
			'name_admin_bar'     => sprintf( _x( '%s', 'add new on admin bar', 'kellomuseo-plugin' ), $this->post_label_singular ),
			'add_new'            => sprintf( _x( 'Add New', 'add new item', 'kellomuseo-plugin' ), $this->post_label_singular ),
			'add_new_item'       => sprintf( __( 'Add New %s', 'kellomuseo-plugin' ), $this->post_label_singular ),
			'new_item'           => sprintf( __( 'New %s', 'kellomuseo-plugin' ), $this->post_label_singular ),
			'edit_item'          => sprintf( __( 'Edit %s', 'kellomuseo-plugin' ), $this->post_label_singular ),
			'view_item'          => sprintf( __( 'View %s', 'kellomuseo-plugin' ), $this->post_label_plural ),
			'all_items'          => sprintf( __( 'All %s', 'kellomuseo-plugin' ), $this->post_label_plural ),
			'search_items'       => sprintf( __( 'Search %s', 'kellomuseo-plugin' ), $this->post_label_plural ),
			'parent_item_colon'  => sprintf( __( 'Parent %s:', 'kellomuseo-plugin' ), $this->post_label_plural ),
			'not_found'          => sprintf( __( 'No %s found.', 'kellomuseo-plugin' ), $this->post_label_plural ),
			'not_found_in_trash' => sprintf( __( 'No %s found in Trash.', 'kellomuseo-plugin' ), $this->post_label_plural ),
		);
